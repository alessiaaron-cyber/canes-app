...
(() => {
  ...
  const pointsFor = (pick) => (pick.goals * 2) + pick.assists + (pick.firstGoal ? 2 : 0);
  ...
  function renderHero() {
    ...
    const users = pregame ? pregameUsers : (finalMode ? final.users : live.users);
    ...
    const subline = pregame
      ? (nextDraftSide() ? `${nextDraftSide()} on the clock • Pick ${pregameUsers.Aaron.length + pregameUsers.Julie.length + 1} of 4` : 'Picks ready for puck drop')
      : (liveMode ? (isPlayoffs() ? 'Playoff rivalry scoring live' : 'Rivalry scoring live') : '');

    return `
      <section class="gd-hero ${finalMode ? 'gd-hero-final' : ''}">
        <div class="gd-status-row">
          <span class="gd-pill ${finalMode ? 'final' : 'live'}">${finalMode ? 'Final' : (liveMode ? 'Live' : 'Pregame')}</span>
          ${period ? `<span class="gd-period">${period}</span>` : ''}
          ${liveMode ? '<span class="gd-pill synced">Synced</span>' : ''}
          ${isPlayoffs() ? '<span class="gd-pill gd-pill-playoff">Playoffs</span>' : ''}
        </div>
        <div class="gd-score-grid">
          <div class="gd-side">
            <div class="gd-side-label red">Aaron</div>
            ${pregame ? `<div class="gd-pick-meta"><span class="gd-pick-chip ${pregameUsers.Aaron.length === 2 ? 'active' : ''}">${pregameUsers.Aaron.length} of 2 locked</span></div>` : `<div class="gd-side-value">${scores.Aaron}</div>`}
          </div>
          <div class="gd-center"><img class="gd-logo" src="./assets/app-icon.png" alt="Canes Rivalry"></div>
          <div class="gd-side">
            <div class="gd-side-label">Julie</div>
            ${pregame ? `<div class="gd-pick-meta"><span class="gd-pick-chip ${pregameUsers.Julie.length === 2 ? 'active' : ''}">${pregameUsers.Julie.length} of 2 locked</span></div>` : `<div class="gd-side-value">${scores.Julie}</div>`}
          </div>
        </div>
        ${subline ? `<div class="gd-subline">${subline}</div>` : ''}
        ${finalMode ? `<div class="gd-final-banner">${winnerText(scores)}</div>` : ''}
        ${liveMode ? `<div class="gd-momentum-label">Momentum</div><div class="gd-track"><div class="gd-track-fill" style="left:${scores.Aaron >= scores.Julie ? '50%' : `calc(50% - ${momentum}%)`};width:${momentum}%"></div></div>` : ''}
      </section>
    `;
  }
  ...
  function renderPregame() {
    ...
    const filteredRoster = roster.filter((entry) => entry.name.toLowerCase().includes(CR.gameDay.search.toLowerCase()));

    const renderOwner = (side, red) => `
      <article class="gd-panel">
        <div class="gd-panel-head ${red ? 'red' : 'dark'}"><span>${side}</span><span>${users[side].length}/2</span></div>
        ${[0, 1].map((index) => {
          const pick = users[side][index];
          return pick
            ? `<div class="gd-pick-row"><div class="gd-pick-icon">✓</div><div class="gd-pick-main"><strong>${pick.player}</strong><small>Locked pick</small><div class="gd-lock-actions"><button class="gd-small-action" data-side="${side}" data-player="${pick.player}" type="button">Change</button></div></div></div>`
    ...
    return `
      ...
      <section class="gd-picks-grid">
        ${renderOwner('Aaron', true)}
        ${renderOwner('Julie', false)}
      </section>
      <section class="gd-search"><input class="gd-search-input" id="pregameSearch" placeholder="Search current Canes roster..." value="${CR.gameDay.search}" /></section>
      <div class="gd-label-row"><div class="gd-label">Current Canes Roster</div><div class="gd-filter">Tap Draft only</div></div>
      <section class="gd-panel gd-roster gd-scroll">
        ${filteredRoster.map((entry) => {
          ...
          return `
            <div class="gd-roster-row ${owner ? 'claimed' : ''}">
              ...
              ${owner ? `<span class="gd-tag">${owner}</span>` : `<button class="gd-draft-btn" data-player="${entry.name}" type="button">Draft</button>`}
            </div>
          `;
        }).join('')}
      </section>
  ...
  const renderPlayerCard = (side, picks, score, red) => `
    <article class="gd-card">
      <div class="gd-pick-card-head"><strong class="${red ? 'red' : ''}">${side}</strong><span>${score} pts</span></div>
      ${picks.map((pick) => `
  ...
  function renderLive() {
    ...
    return `
      <div class="gd-label-row"><div class="gd-label">Picked Players</div><button class="gd-manage-tiny" data-action="open-manage" type="button">Manage</button></div>
      <section class="gd-picks-grid">
        ${renderPlayerCard('Aaron', state.users.Aaron, state.scores.Aaron, true)}
        ${renderPlayerCard('Julie', state.users.Julie, state.scores.Julie, false)}
      </section>
    ...
    return `
      <section class="gd-card gd-postgame-card">
        <div class="gd-postgame-top">
          ...
          <div><div class="gd-postgame-title">Postgame Summary</div><div class="gd-postgame-sub">How the night was won.</div></div>
        </div>
        <div class="gd-postgame-grid">
          <div class="gd-postgame-pill"><strong>MVP</strong><span>${mvpText(state.users)}</span></div>
          <div class="gd-postgame-pill"><strong>Edge</strong><span>${leadingStatType(state.users)}</span></div>
          ...
          <div class="gd-postgame-pill"><strong>Total Events</strong><span>${totalGoals(state.users)} goals • ${totalAssists(state.users)} assists</span></div>
        </div>
      </section>
      <div class="gd-label-row"><div class="gd-label">Final Pick Breakdown</div><button class="gd-manage-tiny" data-action="open-manage" type="button">Manage</button></div>
    ...
    actions.innerHTML = ['Aaron', 'Julie'].flatMap((side) => [0, 1].map((index) => {
      ...
      return `
        <div class="gd-sheet-pick">
          ...
          <select class="gd-sheet-select" data-side="${side}" data-index="${index}">${opts}</select>
        </div>
      `;
    })).join('');
  ...
  function bindInteractions() {
    $('#pregameSearch')?.addEventListener('input', (event) => {
      CR.gameDay.search = event.target.value;
    ...
    document.querySelectorAll('[data-action="open-manage"]').forEach((button) => {
      button.addEventListener('click', () => {
        renderManageSheet();
        $('#manageSheet')?.classList.add('is-open');
      });
    });
  ...
  CR.initGameDay = () => {
    ...
    $('#closeSheet')?.addEventListener('click', () => $('#manageSheet')?.classList.remove('is-open'));
