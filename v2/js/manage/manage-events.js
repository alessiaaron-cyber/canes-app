...
(() => {
  ...
  function closeAllSheets() {
    ...
    CR.manageState.rosterSheetOpen = false;
    CR.manageState.scheduleSheetOpen = false;
    CR.manageState.confirmRemove = null;
  }
  ...
  function bindManageEvents() {
    ...
    root.addEventListener('input', (event) => {
      ...
      if (newSeasonInput) { CR.manageState.newSeasonDraft.seasonLabel = newSeasonInput.value; return; }
      const rosterInput = event.target.closest('[data-manage-roster-input]');
      if (rosterInput) { CR.manageState.rosterDraft[rosterInput.dataset.manageRosterInput] = rosterInput.value; return; }
      const scheduleInput = event.target.closest('[data-manage-schedule-input]');
      if (scheduleInput) CR.manageState.scheduleDraft[scheduleInput.dataset.manageScheduleInput] = scheduleInput.value;
    });

    root.addEventListener('click', (event) => {
      const viewTrigger = event.target.closest('[data-manage-view]');
      if (viewTrigger) { closeAllSheets(); CR.manageState.activeManageView = viewTrigger.dataset.manageView || 'main'; rerender({ scrollTop: true }); return; }

      const openPlayerSheet = event.target.closest('[data-manage-open-player-sheet]');
      if (openPlayerSheet) { closeAllSheets(); resetRosterDraft(); CR.manageState.rosterSheetOpen = true; rerender(); return; }

      const closePlayerSheet = event.target.closest('[data-manage-close-player-sheet]');
      if (closePlayerSheet) { resetRosterDraft(); CR.manageState.rosterSheetOpen = false; rerender(); return; }
      ...
      if (editPlayer) {
        ...
        if (player) {
          ...
          CR.manageState.rosterSheetOpen = true;
          rerender();
        }
        return;
      }
      ...
      if (savePlayer) {
        ...
        if (CR.manageState.editingRosterPlayerId) {
          ...
          resetRosterDraft(); CR.manageState.rosterSheetOpen = false; rerender(); CR.showToast?.({ message: `${name} updated` }); return;
        }
        CR.manageState.roster.push({ id: makeId('player'), ...payload, active: true });
        resetRosterDraft(); CR.manageState.rosterSheetOpen = false; rerender(); CR.showToast?.({ message: `${name} added` }); return;
      }

      const openGameSheet = event.target.closest('[data-manage-open-game-sheet]');
      if (openGameSheet) { closeAllSheets(); resetScheduleDraft(); CR.manageState.scheduleSheetOpen = true; rerender(); return; }

      const closeGameSheet = event.target.closest('[data-manage-close-game-sheet]');
      if (closeGameSheet) { resetScheduleDraft(); CR.manageState.scheduleSheetOpen = false; rerender(); return; }
      ...
      if (editGame) {
        ...
        if (game) {
          ...
          CR.manageState.scheduleDraft = { date: game.date, opponent: game.opponent, type: game.type, firstPicker: game.firstPicker };
          CR.manageState.scheduleSheetOpen = true;
          rerender();
        }
        return;
      }
      ...
      if (saveGame) {
        ...
        const payload = { date, opponent, type: draft.type || 'Regular', firstPicker: draft.firstPicker || CR.manageState.season.firstPicker };
        if (CR.manageState.editingScheduleGameId) {
          ...
          resetScheduleDraft(); CR.manageState.scheduleSheetOpen = false; rerender(); CR.showToast?.({ message: `${opponent} game updated` }); return;
        }
        CR.manageState.schedule.push({ id: makeId('game'), ...payload });
        resetScheduleDraft(); CR.manageState.scheduleSheetOpen = false; rerender(); CR.showToast?.({ message: `${opponent} game added` }); return;
      }

      const confirmRemovePlayer = event.target.closest('[data-manage-confirm-remove-player]');
      if (confirmRemovePlayer) {
        const player = CR.manageState.roster.find((item) => item.id === confirmRemovePlayer.dataset.manageConfirmRemovePlayer);
        if (player) { closeAllSheets(); CR.manageState.confirmRemove = { type: 'player', id: player.id, label: player.name }; rerender(); }
        return;
      }

      const confirmRemoveGame = event.target.closest('[data-manage-confirm-remove-game]');
      if (confirmRemoveGame) {
        const game = CR.manageState.schedule.find((item) => item.id === confirmRemoveGame.dataset.manageConfirmRemoveGame);
        if (game) { closeAllSheets(); CR.manageState.confirmRemove = { type: 'game', id: game.id, label: `${game.date} · ${game.opponent}` }; rerender(); }
        return;
      }

      const cancelRemove = event.target.closest('[data-manage-cancel-remove]');
      if (cancelRemove) { CR.manageState.confirmRemove = null; rerender(); return; }

      const confirmRemove = event.target.closest('[data-manage-confirm-remove]');
      if (confirmRemove) {
        const item = CR.manageState.confirmRemove;
        if (item?.type === 'player') CR.manageState.roster = CR.manageState.roster.filter((player) => player.id !== item.id);
        if (item?.type === 'game') CR.manageState.schedule = CR.manageState.schedule.filter((game) => game.id !== item.id);
        CR.manageState.confirmRemove = null;
        rerender();
      ...
      if (startSeason) { closeAllSheets(); CR.manageState.startSeasonOpen = true; rerender(); return; }
      const closeStartSeason = event.target.closest('[data-manage-close-start-season]');
      if (closeStartSeason) { CR.manageState.startSeasonOpen = false; rerender(); return; }
      const newSeasonPicker = event.target.closest('[data-manage-new-season-picker]');
      if (newSeasonPicker) { CR.manageState.newSeasonDraft.firstPicker = newSeasonPicker.dataset.manageNewSeasonPicker; rerender(); return; }
      ...
      if (confirmStartSeason) {
        ...
        resetScheduleDraft(); CR.manageState.startSeasonOpen = false; rerender(); CR.showToast?.({ message: `${seasonLabel} season started` }); return;
      }

      const editScoring = event.target.closest('[data-manage-edit-scoring]');
      if (editScoring) { closeAllSheets(); CR.manageState.scoringEditOpen = true; rerender(); return; }
      const closeScoring = event.target.closest('[data-manage-close-scoring]');
      if (closeScoring) { CR.manageState.scoringEditOpen = false; rerender(); return; }
      ...
      if (editTrigger) { closeAllSheets(); CR.manageState.activeEditField = editTrigger.dataset.manageEdit; rerender(); return; }
      const closeEdit = event.target.closest('[data-manage-close-edit]');
      if (closeEdit) { CR.manageState.activeEditField = null; rerender(); return; }
      ...
      if (editOption) {
        ...
        if (field && Object.prototype.hasOwnProperty.call(CR.manageState.season, field)) { CR.manageState.season[field] = value; CR.manageState.activeEditField = null; rerender(); CR.showToast?.({ message: `${value} selected` }); }
        return;
      }

      const importSchedule = event.target.closest('[data-manage-import-schedule]');
      if (importSchedule) { CR.showToast?.({ message: 'Mock NHL schedule sync complete' }); return; }
      ...
      if (streamOption) { const nextValue = streamOption.dataset.manageStreamOption; CR.manageState.streamMode.selected = nextValue; rerender(); CR.showToast?.({ message: `Stream Mode set to ${labelForStreamOption(nextValue)}` }); }
    });
  }
