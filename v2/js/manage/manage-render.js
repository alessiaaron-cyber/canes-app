...
(() => {
  ...
  function renderSubviewHeader(label, title, copy) {
    return `<section class="panel-card cr-subpage-hero manage-subview-hero"><button class="cr-button back cr-back-button" type="button" data-manage-view="main">← Manage</button><span class="cr-pill dark">${escapeHtml(label)}</span><h2>${escapeHtml(title)}</h2><p>${escapeHtml(copy)}</p></section>`;
  }
  ...
  function renderRosterView(state) {
    ...
    const heading = isEditing ? 'Edit Player' : 'Add Player';
    const helper = isEditing ? 'Update this player for future picks while history remains protected.' : '';
    return `<div class="content-stack manage-stack">${renderSubviewHeader('Roster', 'Roster', 'Active pick list for future games. Deactivated players remain in history.')}<section class="panel-card manage-card">${renderCardHeader(isEditing ? 'Editing roster' : 'Add player', heading, helper, isEditing ? { className: 'warning', label: 'Editing' } : null)}<div class="cr-form-grid"><label><span class="eyebrow">Name</span><input class="cr-input" value="${escapeHtml(state.rosterDraft.name)}" placeholder="Player name" data-manage-roster-input="name" /></label><label><span class="eyebrow">Position</span><select class="cr-input" data-manage-roster-input="position"><option ${state.rosterDraft.position === 'F' ? 'selected' : ''}>F</option><option ${state.rosterDraft.position === 'D' ? 'selected' : ''}>D</option></select></label></div><button class="cr-button save" type="button" data-manage-save-player>${actionLabel}</button>${isEditing ? '<button class="cr-button secondary full manage-cancel-edit" type="button" data-manage-cancel-edit-player>Cancel edit</button>' : ''}</section><section class="panel-card manage-card">${renderCardHeader('Active players', 'Roster list', '', { className: 'neutral', label: `${state.roster.filter((p) => p.active).length}` })}<div class="cr-list-stack">${state.roster.map((player) => `<article class="cr-action-row ${!player.active ? 'is-muted' : ''} ${state.editingRosterPlayerId === player.id ? 'is-editing' : ''}"><div class="cr-action-copy"><strong>${escapeHtml(player.name)}</strong><span>${escapeHtml(player.position)} · ${player.active ? 'Active' : 'Inactive'}</span></div><div class="manage-row-actions"><button class="cr-button edit" type="button" data-manage-edit-player="${escapeHtml(player.id)}">Edit</button><button class="cr-button ${player.active ? 'remove' : 'secondary'}" type="button" data-manage-toggle-player="${escapeHtml(player.id)}">${player.active ? 'Remove' : 'Restore'}</button></div></article>`).join('')}</div></section></div>`;
  }
