...
const states = {
  missing: {
    ...
    impact: 'No rivalry points yet',
    moments: ['⏳ Aaron has submitted 2/2 picks', '⚠️ Julie still needs one more pick', '🔓 Picks remain editable until scoring starts'],
  ...
  pregame: {
    ...
    impact: 'No rivalry points yet',
    moments: ['✅ Aaron submitted 2/2 picks', '✅ Julie submitted 2/2 picks', '🚫 No overlapping players'],
  ...
  live: {
    ...
    subline: '2nd Period · CAR leads 2-1',
    score: ['2', '1'],
    picksTitle: 'Picks With Stats',
    picksBadge: 'Locked',
    picksBadgeClass: 'live',
    picksNote: 'Goals, assists, and first-goal bonus update live.',
    mode: 'locked',
    latest: 'Svechnikov assist',
    impact: 'Aaron +1 · lead grows to 4-1 tonight',
    moments: ['🚨 Sebastian Aho scored first — Aaron +3', '🍎 Andrei Svechnikov assist — Aaron +1', '🍎 Seth Jarvis assist — Julie +1'],
    users: {
      Aaron: [{ player: 'Sebastian Aho', g: 1, a: 0, first: true, pts: 3 }, { player: 'Andrei Svechnikov', g: 0, a: 1, first: false, pts: 1 }],
      Julie: [{ player: 'Seth Jarvis', g: 0, a: 1, first: false, pts: 1 }, { player: 'Jaccob Slavin', g: 0, a: 0, first: false, pts: 0 }]
    }
  },
  final: {
    title: 'Final Result',
    ...
    subline: 'Final · Hurricanes win 3-2',
    score: ['3', '2'],
    ...
    mode: 'locked',
    latest: 'Game finalized',
    impact: 'Aaron wins 5-2 on player picks',
    moments: ['🏁 Finalized: Aaron wins 5-2', '🚨 First-goal bonus mattered: Aho +3 total', '📈 Aaron extends the rivalry lead'],
    users: {
      Aaron: [{ player: 'Sebastian Aho', g: 1, a: 1, first: true, pts: 4 }, { player: 'Andrei Svechnikov', g: 0, a: 1, first: false, pts: 1 }],
      Julie: [{ player: 'Seth Jarvis', g: 1, a: 0, first: false, pts: 2 }, { player: 'Jaccob Slavin', g: 0, a: 0, first: false, pts: 0 }]
    }
  }
};
...
function renderLockedPick(userName, picks) {
  ...
  const rows = picks.map((pick) => {
    ...
    return `<article class="locked-pick-row ${pick.pts > 0 ? 'active-pick' : ''}" data-player="${pick.player}"><div class="locked-pick-main"><strong>${pick.player}</strong><span>${tags.join(' · ')}</span></div><b>+${pick.pts}</b></article>`;
  }).join('');
...
function renderState(mode) {
  ...
  canesScore.textContent = state.score[0];
  oppScore.textContent = state.score[1];
  ...
  eventImpact.textContent = state.impact;
