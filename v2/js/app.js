...
function renderEditablePick(userName, picks, taken, state) {
  const rows = [0, 1].map((index) => {
    ...
    return `<label class="pick-select-row ${!current ? 'missing-pick-row' : ''}"><span>${label}</span><select aria-label="${userName} pick ${index + 1}"><option value="">Choose player</option>${options}</select></label>`;
  }).join('');
  const readyCount = picks.filter(Boolean).length;
  return `<div class="single-user-card ${getUserCardClass(userName, state, picks)}"><div class="single-user-head"><div><strong>${userName}</strong><span>${readyCount}/2 selected</span></div><span class="pick-lock-pill ${readyCount < 2 ? 'warning-pill' : ''}">${readyCount < 2 ? 'Needs pick' : 'Ready'}</span></div>${rows}</div>`;
}
...
function renderLockedPick(userName, picks, state) {
  ...
  return `<div class="single-user-card ${getUserCardClass(userName, state, [])}"><div class="single-user-head"><div><strong>${userName}</strong><span>${picks.length}/2 locked</span></div><div class="score-stack">${winnerBadge}<span class="owner-score">+${total}</span></div></div>${rows}</div>`;
}
...
function runAaronScoreSwing() {
  ...
  flashSync();
  addMoment('🔥 Aho swing: Aaron gets the scoring bump');
  document.querySelector('[data-player="Sebastian Aho"]')?.classList.add('pick-hit');
  showToast('Aho swing · Aaron +3');
}

function runJulieAssist() {
  if (currentMode === 'missing' || currentMode === 'pregame') renderState('live');
  flashSync();
  addMoment('👀 Jarvis answer: Julie stays alive');
