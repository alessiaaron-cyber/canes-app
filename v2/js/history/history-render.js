...
(() => {
  ...
  function renderBoard(data) {
    ...
    return `<section class="panel-card rivalry-board-card history-legacy-card"><div class="rivalry-board-topline"><span class="eyebrow">Rivalry History</span><div class="history-status-cluster"><span class="history-meta-pill history-games-pill">${escapeHtml(String(board.totalGames || 0))} games logged</span><span class="history-status-pill ${leader.className}">${escapeHtml(status)}</span></div></div><h2 class="rivalry-board-title">${escapeHtml(board.lead || 'Rivalry tied')}</h2><div class="history-scoreboard-banner"><div class="history-scoreboard-side"><span class="history-scoreboard-name ${userThemeClass(data, 0)}">${escapeHtml(userName(data, 0))}</span></div><div class="history-scoreboard-center"><strong>${escapeHtml(String(board.aaron ?? 0))}</strong><span aria-hidden="true">—</span><strong>${escapeHtml(String(board.julie ?? 0))}</strong></div><div class="history-scoreboard-side is-right"><span class="history-scoreboard-name ${userThemeClass(data, 1)}">${escapeHtml(userName(data, 1))}</span></div></div><p class="history-hero-copy">${leader.margin ? `${escapeHtml(leader.name)} holds the all-time edge, but every game still moves the line.` : 'The all-time race is tied. Every game is a chance to grab the lead.'}</p></section>`;
  }
