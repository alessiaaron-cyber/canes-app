...
(() => {
  ...
  function renderSeasonCard(data, summary) {
    ...
    return `<button class="history-season-overview-card ${winnerClass}" type="button" data-history-open-season="${escapeHtml(summary.seasonId)}" aria-label="View ${escapeHtml(summary.label || season?.label || summary.seasonId)} season details"><div class="history-season-overview-topline"><div><div class="eyebrow">${escapeHtml(season?.isCurrent ? 'Current season' : 'Season')}</div><h3>${escapeHtml(summary.label || season?.label || summary.seasonId)}</h3></div><span class="history-season-view-cue" aria-hidden="true">›</span></div><div class="history-season-overview-score"><div class="history-season-overview-side"><span class="history-season-overview-name">Aaron</span><strong>${escapeHtml(String(totals.aaron))}</strong></div><div class="history-season-overview-divider" aria-hidden="true">—</div><div class="history-season-overview-side is-right"><span class="history-season-overview-name">Julie</span><strong>${escapeHtml(String(totals.julie))}</strong></div></div><div class="history-season-overview-meta"><span class="history-season-overview-tag neutral">Record ${escapeHtml(summary.recordText || '—')}</span><span class="history-season-overview-note">${escapeHtml(String(games.length))} games</span><span class="history-season-overview-note">${escapeHtml(playoffCount ? `${playoffCount} playoff games` : 'No playoff games')}</span></div>${summary.bestGameTitle ? `<p class="history-meta-note">Best game: ${escapeHtml(summary.bestGameTitle)}</p>` : ''}</button>`;
  }
