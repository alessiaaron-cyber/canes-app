...
(() => {
  ...
  function renderBoard(data) {
    ...
    return `<section class="panel-card rivalry-board-card"><div class="rivalry-board-topline"><span class="eyebrow">All-time rivalry</span></div><h2 class="rivalry-board-title">${escapeHtml(board.lead || 'Rivalry tied')}</h2><div class="rivalry-board-score-grid"><article class="rivalry-score-card"><div class="eyebrow">Aaron</div><div class="rivalry-score-value">${escapeHtml(String(board.aaron ?? 0))}</div></article><article class="rivalry-score-card"><div class="eyebrow">Julie</div><div class="rivalry-score-value">${escapeHtml(String(board.julie ?? 0))}</div></article></div><div class="rivalry-board-meta-row"><span class="cr-pill neutral">${escapeHtml(String(board.totalGames || 0))} total games</span></div></section>`;
  }

  function renderSeasonSnapshot(data) {
    const board = data.seasonBoard || {};
    return `<section class="panel-card history-hq-card"><div class="history-hq-topline"><span class="eyebrow">Current season view</span><button class="cr-button secondary" type="button" data-history-access="seasons">View All</button></div><h3 class="history-hq-title">${escapeHtml(board.seasonLabel || 'Season')}</h3><div class="history-season-field"><label class="eyebrow" for="historySeasonSelect">Season</label><select id="historySeasonSelect" class="history-season-select">${(data.seasons || []).map((season) => `<option value="${escapeHtml(season.id)}" ${data.selectedSeason?.id === season.id ? 'selected' : ''}>${escapeHtml(season.label)}</option>`).join('')}</select></div><div class="history-season-score-grid"><article class="rivalry-score-card"><div class="eyebrow">Aaron</div><div class="rivalry-score-value">${escapeHtml(String(board.aaron ?? 0))}</div></article><article class="rivalry-score-card"><div class="eyebrow">Julie</div><div class="rivalry-score-value">${escapeHtml(String(board.julie ?? 0))}</div></article></div><div class="history-season-meta-row"><span class="cr-pill neutral">Record ${escapeHtml(board.recordText || '—')}</span><span class="cr-pill neutral">${escapeHtml(board.recentText || 'Recent form unavailable')}</span><span class="cr-pill neutral">Best game ${escapeHtml(board.bestGameTitle || '—')}</span></div></section>`;
  }
  ...
  function renderSeasonCard(data, summary) {
    ...
    return `<button class="history-season-overview-card ${winnerClass}" type="button" data-history-open-season="${escapeHtml(summary.seasonId)}"><div class="history-season-overview-topline"><div><div class="eyebrow">${escapeHtml(season?.isCurrent ? 'Current season' : 'Season')}</div><h3>${escapeHtml(summary.label || season?.label || summary.seasonId)}</h3></div></div><div class="history-season-overview-score"><div class="history-season-overview-side"><span class="history-season-overview-name">Aaron</span><strong>${escapeHtml(String(totals.aaron))}</strong></div><div class="history-season-overview-divider">vs</div><div class="history-season-overview-side is-right"><span class="history-season-overview-name">Julie</span><strong>${escapeHtml(String(totals.julie))}</strong></div></div><div class="history-season-overview-meta"><span class="history-season-overview-tag ${winnerClass}">${escapeHtml(summary.recordText || 'Season record unavailable')}</span><span class="history-season-overview-note">${escapeHtml(String(games.length))} games</span><span class="history-season-overview-note">${escapeHtml(playoffCount ? `${playoffCount} playoff games` : 'No playoff games')}</span>${summary.bestGameTitle ? `<span class="history-season-overview-note">Best game: ${escapeHtml(summary.bestGameTitle)}</span>` : ''}</div></button>`;
  }
