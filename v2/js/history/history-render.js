...
(() => {
  ...
  function renderSeasonCard(data, summary) {
    ...
    return `<button class="history-season-overview-card ${winnerClass}" type="button" data-history-open-season="${escapeHtml(summary.seasonId)}"><div class="history-season-overview-topline"><div><div class="eyebrow">${escapeHtml(season?.isCurrent ? 'Current season' : 'Season')}</div><h3>${escapeHtml(summary.label || season?.label || summary.seasonId)}</h3></div><span class="cr-pill neutral">${escapeHtml(String(games.length))} games</span></div><div class="history-season-overview-score"><div class="history-season-overview-side"><span class="history-season-overview-name">Aaron</span><strong>${escapeHtml(String(totals.aaron))}</strong></div><div class="history-season-overview-divider">vs</div><div class="history-season-overview-side is-right"><span class="history-season-overview-name">Julie</span><strong>${escapeHtml(String(totals.julie))}</strong></div></div><div class="history-season-overview-meta"><span class="history-season-overview-tag ${winnerClass}">${escapeHtml(summary.recordText || 'Season record unavailable')}</span><span class="history-season-overview-note">${escapeHtml(playoffCount ? `${playoffCount} playoff games` : 'No playoff games')}</span>${summary.bestGameTitle ? `<span class="history-season-overview-note">Best game: ${escapeHtml(summary.bestGameTitle)}</span>` : ''}</div></button>`;
  }
